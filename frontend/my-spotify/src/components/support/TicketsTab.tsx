...
export default function TicketsTab({user} : {user: UserProfile}) {
  ...
  if (selectedTicket) {
    ...
    return (
      <div className="h-full flex flex-col max-w-3xl mx-auto px-4 sm:px-0 animate-fade-in space-y-6 bg-black">
        <div className="flex items-center justify-between bg-[#0a0a0a] border border-neutral-800/60 p-4 sm:p-5 rounded-2xl shrink-0 gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <h2 className="text-sm sm:text-base font-bold text-white tracking-tight truncate">{selectedTicket.subject}</h2>
        ...
        <div className="bg-[#121212] border border-neutral-800/60 rounded-2xl p-5 sm:p-6 relative overflow-hidden">
        ...
        {isAnswered ? (
          <div className="bg-neutral-950 border border-neutral-900 rounded-2xl p-5 sm:p-6 relative">
            <div className="flex flex-wrap items-center gap-2 text-[10px] sm:text-xs font-bold text-neutral-400 font-mono border-b border-neutral-900 pb-3 mb-4">
          ...
          <div className="bg-[#121212] border border-neutral-800/60 rounded-2xl p-5 sm:p-6 flex flex-col space-y-4">
        ...
        {tickets.length === 0 ? (
          ...
          <table className="w-full text-left text-sm text-neutral-400 table-fixed">
            <thead className="bg-neutral-900/50 text-xs uppercase font-semibold text-neutral-500 border-b border-neutral-800/50">
              <tr>
                <th className="px-4 py-4 w-[70px] hidden md:table-cell">Ticket ID</th>
                <th className="px-4 sm:px-6 py-4 w-[35%] md:w-[25%]">Username</th>
                <th className="px-4 sm:px-6 py-4 w-[45%] md:w-[40%]">Subject</th>
                <th className="px-6 py-4 w-[20%] hidden md:table-cell">Date</th>
                <th className="px-4 sm:px-6 py-4 w-[20%] md:w-[15%] text-right">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-800/50">
              {tickets.map((ticket) => (
                ...
                >
                  ...
                  <td className="px-4 sm:px-6 py-4 font-semibold text-white truncate">
                    @{ticket.username}
                  </td>
                  <td className="px-4 sm:px-6 py-4 truncate text-neutral-300">
                    {ticket.subject}
                  </td>
                  <td className="px-6 py-4 text-neutral-500 hidden md:table-cell truncate">
...
function StatusBadge({ status }: { status: TicketStatus }) {
  switch (status) {
    case 'Open':
      return <span className="text-[9px] bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 font-mono font-bold px-1.5 sm:px-2 py-0.5 rounded uppercase tracking-wider">Open</span>;
    case 'Replied':
      return <span className="text-[9px] bg-neutral-800 text-neutral-400 border border-neutral-700/50 font-mono font-bold px-1.5 sm:px-2 py-0.5 rounded uppercase tracking-wider">Closed</span>;
    case 'Closed':
      return <span className="text-[9px] bg-neutral-900 text-neutral-600 border border-transparent font-mono font-bold px-1.5 sm:px-2 py-0.5 rounded uppercase tracking-wider">Closed</span>;
    default:
      return null;
  }
}